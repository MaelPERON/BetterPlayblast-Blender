...
from .utils.sanity_checks import sanity_file_saved, sanity_file_stem
...
class BP_Preferences(bpy.types.AddonPreferences):
    ...
    pb_folder = bpy.props.EnumProperty(  # type: ignore
        ...
        items=[
            ...
            ("RENDER", "Same as Output Path",
             "Same Folder as the Output Path under Render Settings")
        ],
        default="RENDER"
    )
    ...
    pb_folder_force = bpy.props.BoolProperty(  # type: ignore
        ...
        description=(
            "When folder doesn't exist, create it.\n"
    ...
    def get_vs_presets(self, context):
        ...
        ffmpeg_preset_folder = Path(
            bpy.utils.resource_path("LOCAL")) / "scripts/presets/ffmpeg"
        if ffmpeg_preset_folder.exists():
            for preset in ffmpeg_preset_folder.glob("*.py"):
                items.append(
                    (preset.name,
                     preset.stem,
                     f"Load preset at file location: {preset.resolve()}"))
        return items
    ...
    def draw(self, context: bpy.types.Context):
        ...
        match self.pb_folder:
            ...
            case "RENDER":
                ...
                self.pb_folder_render = str(
                    filepath) if filepath.parent.exists() else ""
        ...
        self.pb_filename_render = bpy.path.basename(
        ...
        if preview_path:
            ...
            row.prop(
                self,
                "pb_path_preview",
                text="Playblast Destination Preview",
        ...
        spawn_warning(
            box,
            "Video Codec not supported for the moment. Default to .MPEG-4",
        )
        box.separator()
        col = box.column(align=True)
        ...
        row.label(
            text=(
                "If selected, the video settings will be overriden to "
                "the selected ffmpeg preset."
            )
        )
        if self.vs_presets != "NONE":
            spawn_warning(
                col,
                (
                    "Using a preset will override the video settings below. "
                    "Make sure to check the preset file for details."
                ),
            )
