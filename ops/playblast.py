...
from ..utils.capture_data import data_init_capture, data_frame_capture
...
class BP_Playblast(bpy.types.Operator):
    ...
    preview_process = bpy.props.BoolProperty(  # type: ignore
        name="Preview Process",
        description="Display the video overlay preview when building it.",
        default=False,
    )
    ...
    def execute(self, context: bpy.types.Context):
        if Playblast is None:
            self.report(
                {'ERROR'},
                "Better Playblast is not installed or not available.")
            return {'CANCELLED'}

        # Configure path local_app_data
        # not compatible with non-Windows OS
